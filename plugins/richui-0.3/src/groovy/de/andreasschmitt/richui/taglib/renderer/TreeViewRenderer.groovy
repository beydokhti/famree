...
class TreeViewRenderer extends AbstractRenderer {
	...
	protected void renderTagContent(Map attrs, GroovyPageTagBody body, MarkupBuilder builder) throws RenderException {			
		...
		builder.script(type: "text/javascript"){  		 
		    builder.yieldUnescaped "	var tree = new YAHOO.widget.TreeView(\"$attrs.id\"); var change = true; \n"
		    builder.yieldUnescaped "	tree.subscribe('collapse', function(node) {  if(!change) {change=true;return false;} }) \n" 
		    builder.yieldUnescaped "	tree.subscribe('expand', function(node) {  if(!change) {change=true;return false;} }) \n"
		    ...
		    if(attrs?.onLabelClick){
			    ...
			    builder.yieldUnescaped "		var id = node.additionalId; change = false;"
			...
			builder.yieldUnescaped "	tree.draw();tree.expandAll();\n"
		}
	}
